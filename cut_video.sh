...
for ((i=0; i<${#TIMESTAMPS[@]}; i+=2)); do
    ...
    if [ "$KEEP_AUDIO" = "true" ]; then
        ffmpeg -i "$INPUT_VIDEO" -ss "$START" -to "$END" -c:v hevc_videotoolbox -b:v 5000k -c:a aac "$SEGMENT_FILE" -y
    else
        ffmpeg -i "$INPUT_VIDEO" -ss "$START" -to "$END" -c:v hevc_videotoolbox -b:v 5000k -an "$SEGMENT_FILE" -y
    fi
...
ffmpeg -f concat -safe 0 -i "$SEGMENT_LIST" -c:v hevc_videotoolbox -b:v 5000k -c:a aac "$OUTPUT_VIDEO" -y
